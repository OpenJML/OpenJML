...
public class JavaTokenizer {
    ...
    /** The token factory. */
    protected final Tokens tokens; // DRC - changed from private to protected
     ...
     */
    protected void scanFraction(int pos) { // DRC - changed from private to protected
        skipIllegalUnderscores();
        if ('0' <= reader.ch && reader.ch <= '9') {
            scanDigits(pos, 10);
        }
     ...
     */
    protected void scanFractionAndSuffix(int pos) { // DRC - changed to protected to be able to snag .. between int
        radix = 10;
        scanFraction(pos);
        if (reader.ch == 'f' || reader.ch == 'F') {
            reader.putChar(true);
    ...
    /** Read an identifier.
     */
    protected void scanIdent() { // DRC - changed private to protected
     ...
     */
    protected boolean isSpecial(char ch) { // DRC - private to protected
        switch (ch) {
