...
public class JmlEnter extends Enter {
    ...
    public void visitTopLevel(JCCompilationUnit tree) {
        JmlCompilationUnit jtree = (JmlCompilationUnit)tree;
        modeOfCU = jtree.mode;
        if (modeOfCU == 0) {
            throw new JmlInternalError();
        }

        if (Utils.jmldebug) System.out.println("entering " + jtree.sourcefile.getName());
        // First do all the linking of java types to specifications
        matchAndLinkSpecClasses(jtree);
        //if (Utils.jmldebug) System.out.println("ENTER TOPLEVEL - SUPER " + jtree.sourcefile.getName());
        // Then do all the regular Java registering of packages and types
        super.visitTopLevel(jtree);
        //if (Utils.jmldebug) System.out.println("ENTER TOPLEVEL - JML " + jtree.sourcefile.getName());
        // Then add in any top-level model types
        addTopLevelModelTypes(jtree);
        if (Utils.jmldebug) System.out.println("  completed entering " + jtree.sourcefile.getName());
    }
    ...
    public void addTopLevelModelTypes(JmlCompilationUnit javaCompilationUnit) {
        ...
        if (javaCompilationUnit.specsSequence == null) return;
        ...
        JmlCompilationUnit specCompilationUnit = javaCompilationUnit.specsSequence.get(0);
        ...
        for (JmlClassDecl specTypeDeclaration: specCompilationUnit.parsedTopLevelModelTypes) {
            if (Utils.isJML(specTypeDeclaration.mods)) {
                ...
                JmlSpecs.instance(context).putSpecs(specTypeDeclaration.sym,
                        new JmlSpecs.TypeSpecs(specTypeDeclaration));
            } else {
                log.error("jml.internal.notsobad","Unexpected non-model type declaration is in the list of parsed model types");
            }
        }
    ...
    public void matchAndLinkSpecClasses(JmlCompilationUnit javaCompilationUnit) {
        ...
        if (specsSequence == null) return; // This is the case if the javaCompilationUnit is a spec file instead of the main Java file
        ...
        for (JCTree def: specCompilationUnit.defs) {
            ...
            if (def instanceof JmlClassDecl) {
                ...
                if (Utils.isJML(specTypeDeclaration.mods)) {
                    ...
                    // Also it should not be here - FIXME
                } else {
     ...
     */
    public void matchAndLinkSpecClasses(JmlClassDecl javaTypeDeclaration) {
    ...
    public void visitClassDef(JCClassDecl javaClassDeclaration) {
        ...
        if (JmlCompilationUnit.isSpec(modeOfCU)) specsClassDeclaration = (JmlClassDecl)javaClassDeclaration;
        if (specsClassDeclaration == null) {
            // Record that we found no specs
        ...
        for (JCTree specsNestedDeclaration: specsClassDeclaration.defs) {
            if (specsNestedDeclaration instanceof JmlTypeClauseDecl) {
                ...
                if (specsNestedJmlDecl instanceof JmlClassDecl) {
                    if (Utils.jmldebug) System.out.println("ADDING " + ((JmlClassDecl)specsNestedJmlDecl).name + " TO "  + javaClassDeclaration.name);
    ...
    private int modeOfCU = 0;
    
    public Deque<Env<AttrContext>> binaryMemberTodo = new LinkedList<Env<AttrContext>>();
    ...
    public void enterTopLevelSpecClassesForBinary(ClassSymbol csymbol, java.util.List<JmlCompilationUnit> specsSequence) {
        ...
        JavaFileObject prevSource = log.useSource(specCompilationUnit.sourcefile);
        modeOfCU = specCompilationUnit.mode;
        if (modeOfCU == JmlCompilationUnit.UNKNOWN) throw new JmlInternalError();
...
//                System.out.println("SPEC TYPE SCOPE " + newScope);
                if (Utils.isJML(specTypeDeclaration.mods)) {
                    ...
                    if (matchingCSymbol == null) {
                        ...
                        if (tspecs != null) {
                            ...
                            enterTypeParametersForBinary(matchingCSymbol,specTypeDeclaration,localEnv);
                            // Now recursively match up nested types
        ...
        this.env = prevEnv;
        modeOfCU = JmlCompilationUnit.UNKNOWN;
        
        log.useSource(prevSource);
        if (Utils.jmldebug) System.out.println("ENTER TOPLEVEL (BINARY) - COMPLETE " + csymbol);
    }
        
    public void enterTypeParametersForBinary(ClassSymbol csym, JmlClassDecl specTypeDeclaration, Env<AttrContext> localEnv) {
