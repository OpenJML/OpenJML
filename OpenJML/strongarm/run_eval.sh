...
do
    ...
    ( tail -f strongarm/$TRACE_RAW & echo $! >$EVAL_NAME.apid ) | GREP_COLOR='01;32' egrep --color "STARTING INFERENCE" & 2>/dev/null 

    
    ( tail -f strongarm/$TRACE_RAW & echo $! >$EVAL_NAME.bpid ) | egrep --color "ABORTED" & 2>/dev/null 
