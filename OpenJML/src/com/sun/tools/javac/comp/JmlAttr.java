...
import org.jmlspecs.openjml.JmlTree.JmlRefiningStatement;
...
//        }
    ...
    public void visitJmlModelProgramStatement(JmlModelProgramStatement that) {
      ...
      JmlSpecificationCase modelProgramTree = (JmlSpecificationCase) that.getItem();
      List<JmlMethodClause> modelProgramclauses = modelProgramTree.clauses;
      
        
        JmlRefiningStatement refiningTree = JmlRefiningStatement.getRefiningStatements();
          for (List<? extends JmlSpecificationCase> l = refiningTree.specs.cases; l.nonEmpty(); l = l.tail){
              List<JmlMethodClause> refiningClauses = l.head.clauses;
              int i=0;
              for(JmlMethodClause spcCase : refiningClauses){
                   if(spcCase.toString().compareToIgnoreCase(modelProgramclauses.get(i).toString())==0){
                      i++;   
                   }else{
                       log.error(new JmlTokenizer.DiagnosticPositionSE(that.pos,that.getEndPosition(log.currentSource().getEndPosTable())),"specs statements in Refining and Model Program do not match");
                  }
              }
