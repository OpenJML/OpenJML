...
//                    log.getWriter(WriterKind.NOTICE).println("(Internal error) POSTCONDITION PROBLEM - no typeSpecs stored for " + s);
    ...
    public boolean isAccessible(Env<AttrContext> env, Type site, Symbol sym) {
        ...
        if (specPublicSym == null) {
            specPublicSym = attr.tokenToAnnotationSymbol.get(DefaultJmlTokenKind.SPEC_PUBLIC);
        }
        if (specProtectedSym == null) {
            specProtectedSym = attr.tokenToAnnotationSymbol.get(DefaultJmlTokenKind.SPEC_PROTECTED);
        }
