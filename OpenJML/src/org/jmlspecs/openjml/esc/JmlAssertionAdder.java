    ...
    protected void addPrePostConditions( 
        ...
        // Assume all static invariants and axioms of classes mentioned in the target method
        {
            ...
            for (ClassSymbol bcd : collector.classes) {
                if (bcd == classDecl.sym) continue; // These are done below
        ...
        if (esc) {
            ...
            for (JCTree dd: classDecl.defs) {
                ...
                id = convertJML(id);
                if (specs.isNonNull(d.sym, (Symbol.ClassSymbol)d.sym.owner)) {
...
//            JCExpression e = M.at(pos).JmlQuantifiedExpr(JmlToken.FORALL, List.<JCVariableDecl>of(qd), null, comp);
        ...
        for (JCTree dd: classDecl.defs) {
            ...
            pushBlock();
            // use addInvariants, add parent invariants
                                ...
                                switch (clause.token) {
                                    ...
                                    case SIGNALS:
                                        ...
                                        try {
                                            ...
                                            JCExpression e = convertJML(ex, condition, true);
...
//                        eresult = meth;
    ...
    public void visitJmlQuantifiedExpr(JmlQuantifiedExpr that) {
        if (pureCopy || esc) {
            ...
            try {
                JmlQuantifiedExpr q = M.at(that.pos()).
                    JmlQuantifiedExpr(that.op,convertCopy(that.decls),convertExpr(that.range),convertExpr(that.value));
                q.setType(that.type);
                q.racexpr = convertExpr(that.racexpr);
                result = eresult = q;
            } finally {
                for (JCVariableDecl d: that.decls) {
        ...
        if (that.racexpr != null) {
            result = eresult = convertExpr(that.racexpr); // FIXME: using convertCopy(that.racexpr); fails
        } else {
