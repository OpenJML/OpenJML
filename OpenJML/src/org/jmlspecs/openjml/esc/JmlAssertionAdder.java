...
public class JmlAssertionAdder extends JmlTreeScanner {
    ...
    /** The symbol to use when translating this - starts as the this for the
     * receiver object, but can change as methods or constructors are called.
     */
    protected VarSymbol currentThisSym;
    ...
    /** Maps original AST expression nodes to translated nodes */
    protected Map<JCExpression, JCExpression> nodeTranslation; // FIXME - not used
    ...
    public BiMap<JCTree, JCTree> bimap = new BiMap<JCTree, JCTree>();
    ...
    protected JCBlock convertMethodBodyNoInit(JmlMethodDecl pmethodDecl, JmlClassDecl pclassDecl) {
        ...
        try {
            ...
            {
                JCVariableDecl d = treeutils.makeVarDef(syms.exceptionType,exceptionName,methodDecl.sym,
            ...
            JCBlock b = popBlock(0,methodDecl.body == null ? methodDecl.pos(): methodDecl.body.pos());
    ...
    public @Nullable <T extends JCTree> T convert(@Nullable T tree) {
        ...
        if (!(tree instanceof JCAssign) && !(tree instanceof JCAssignOp)) {
            // The excluded cases have to be handled specially in the visit... methods
            if (result == null) bimap.putf(tree,null);
            else bimap.put(tree, result);
        }
        return (T)result;
    }
    ...
    protected JCStatement addAssert(
        ...
        if (rac) {
            ...
            if (JmlOption.isOption(context, JmlOption.RAC_COMPILE_TO_JAVA_ASSERT)) {
                JCStatement st = M.at(codepos).Assert(expr, treeutils.makeStringLiteral(msg,expr.pos));
                currentStatements.add(st);
                return st;
            } else {
                if (info == null) {
    ...
    protected JmlStatementExpr addAssume(
        ...
        if (rac && racCheckAssumeStatements) {
            addAssert(pos,label,ex,associatedPosition,associatedSource,info);
        }
        return st;
    }
...
//        d.sym.flags_field |= Flags.FINAL;
    ...
    protected void addPrePostConditions( 
        ...
        if (esc) {
            ...
            fa = treeutils.makeSelect(pos, treeutils.makeIdent(methodDecl.pos, currentThisSym), isAllocSym);
            addAssume(methodDecl.pos(),Label.IMPLICIT_ASSUME,fa);

            fa = treeutils.makeSelect(pos, treeutils.makeIdent(methodDecl.pos, currentThisSym), allocSym);
        ...
        // Iterate through parent classes and interfaces, adding relevant axioms and invariants
        ...
        if (rac) {
            ...
            if (!isConstructor) {
                ...
                JCExpression str = treeutils.makeStringLiteral("Runtime exception while evaluating preconditions - preconditions are undefined in JML",methodDecl.pos);
            ...
            
            {
                ...
                vd = treeutils.makeVarDef(syms.runtimeExceptionType, n, methodDecl.sym, methodDecl.pos);
                ex = treeutils.makeIdent(methodDecl.pos,vd.sym);
                JCExpression str = treeutils.makeStringLiteral("Runtime exception while evaluating postconditions - postconditions are undefined in JML",methodDecl.pos);
                JCStatement st = methodCallUtilsStatement(methodDecl.pos(),"reportException",str,ex);
                JCBlock bl = M.at(methodDecl.pos).Block(0, List.<JCStatement>of(st));
                JCCatch catcher = M.at(methodDecl.pos).Catch(vd,bl);
            ...
            if (!isConstructor) {
                ...
                vd = treeutils.makeVarDef(syms.runtimeExceptionType, n, methodDecl.sym, methodDecl.pos);
                ex = treeutils.makeIdent(methodDecl.pos,vd.sym);
                JCExpression str = treeutils.makeStringLiteral("Runtime exception while evaluating exceptional postconditions - signals clauses are undefined in JML",methodDecl.pos);
                JCStatement st = methodCallUtilsStatement(methodDecl.pos(),"reportException",str,ex);
                JCBlock bl = M.at(methodDecl.pos).Block(0, List.<JCStatement>of(st));
                JCCatch catcher = M.at(methodDecl.pos).Catch(vd,bl);
...
//        finalizerstats.add(ifstat);
    ...
    public void visitExec(JCExpressionStatement that) {
        ...
        if (arg != null && (arg instanceof JCMethodInvocation || arg instanceof JCAssign || arg instanceof JCAssignOp) || arg instanceof JCUnary) {
    ...
    public void visitThrow(JCThrow that) {
        ...
        if (javaChecks) addAssert(e.pos(), Label.POSSIBLY_NULL, e);
        ...
        if (that.expr.type.tag != TypeTags.BOT) {
            ...
            JCExpressionStatement assign = M.at(that.pos()).Exec(treeutils.makeAssign(that.pos,id,localid));
            bimap.put(that, assign);
            bimap.put(that, assign.expr);
            addStat(assign);
            // TERMINATION = ???
        ...
        if (pfac instanceof JmlStoreRefKeyword) {
            ...
            if (pfa.sym == null) {
                ...
                if (pfs instanceof Symbol.ClassSymbol) {
                    ...
                    JCExpression result = treeutils.makeEqObject(posp, fa.selected, e);
                    return result;
                }
            }
...
//                    aa.hi == null ? /* FIXME - array length -1 */ : jmlrewriter.translate(aa.hi), paat));
     ...
     */
    protected @Nullable
    JCExpression checkAssignable(JCExpression storeref, JmlSpecificationCase c, VarSymbol baseThisSym, VarSymbol targetThisSym) {
                ...
                switch (mclause.token) {
                    case ASSIGNABLE:
                        ...
                        for (JCExpression pstoreref: pstorerefs) {
                            JCExpression nasg = assignmentAllowed(storeref,pstoreref,baseThisSym,targetThisSym);
    ...
    protected void checkAgainstCallerSpecs(JCExpression scannedItem, JCExpression precondition,
        ...
        for (JmlSpecificationCase c : cases) {
            JCExpression condition = checkAssignable(scannedItem,c,baseThisSym,targetThisSym);
    ...
    protected void applyHelper(JCExpression that) {
        ...
        try {
            ...
            if (esc && newclass != null) {
                ...
                st = treeutils.makeAssignStat(that.pos, fa, treeutils.trueLit); // FIXME _ make a copy of trueLit
                addStat( st );
            }
                ...
                for (JmlSpecificationCase cs : calleeSpecs.cases) {
                    ...
                    if (!translatingJML || rac) {  // FIXME - not quite sure of this guard
                            ...
                            try {
                                if (clause.token == JmlToken.ASSIGNABLE) {
                                    ...
                                    for (JCExpression item: storerefs) {
                                        checkAgainstCallerSpecs(convertJML(item),pre, savedThisSym, newThisSymbol);
                                    }
                                    anyAssignableClauses = true;
                                }
                            } catch (NoModelMethod e) {
                        ...
                        if (!anyAssignableClauses) {
                            ...
                            checkAgainstCallerSpecs(M.at(cs.pos()).JmlStoreRefKeyword(JmlToken.BSEVERYTHING),pre, savedThisSym, newThisSymbol);
                        }
            ...
            if (exceptionDeclCall != null) exsuresStatsOuter.add(M.at(that.pos()).Throw(treeutils.makeIdent(that.pos, exceptionDeclCall.sym)));
            else if (rac) {
...
//    }
    ...
    protected JCExpression addImplicitConversion(DiagnosticPosition pos, Type lhstype, JCExpression rhs) {
        if (rac) return rhs; // RAC handles implicit conversions implicitly
    ...
    public void visitAssign(JCAssign that) {
        ...
        if (that.lhs instanceof JCIdent) {
            ...
            for (JmlSpecificationCase c: specs.getDenestedSpecs(methodDecl.sym).cases) {
                JCExpression check = checkAssignable(lhs,c,currentThisSym,currentThisSym);
                if (check != treeutils.trueLit) {
            ...
            addStat( st );
            bimap.put(that,st.expr);
            result = eresult = lhs;
            bimap.put(that.lhs, eresult);
            ...
            for (JmlSpecificationCase c: specs.getDenestedSpecs(methodDecl.sym).cases) {
                JCExpression check = checkAssignable(fa,c,currentThisSym,currentThisSym);
                if (check != treeutils.trueLit) {
                    DiagnosticPosition pos = c.pos();
                    // FIXME - this is not necessarily the right pos
                    for (JmlMethodClause m : c.clauses) {
            ...
            if (javaChecks) addAssert(array.pos(), Label.POSSIBLY_NULL, e);
            ...
            // FIXME - test this
            for (JmlSpecificationCase c: specs.getDenestedSpecs(methodDecl.sym).cases) {
                JCExpression check = checkAssignable(aa,c,currentThisSym,currentThisSym);
                if (check != treeutils.trueLit) {
                    DiagnosticPosition pos = c.pos();
                    // FIXME - this is not necessarily the right pos
                    for (JmlMethodClause m : c.clauses) {
    ...
    protected void visitAssignopHelper(JCAssignOp that, boolean scanned) {
        ...
        if (lhs instanceof JCIdent) {
            ...
            checkAssignable(methodDecl, lhs, that.pos(), currentThisSym, currentThisSym);
            ...
            addBinaryChecks(that,op,lhs,rhs);
            checkAssignable(methodDecl, lhs, that, currentThisSym, currentThisSym);
    ...
    public void visitUnary(JCUnary that) {
        ...
        if (translatingJML || pureCopy) {
            ...
            JCExpression arg = convertExpr(that.getExpression()); // FIXME - check that this is not converted to a different temporary beofre being incremented
    ...
    public void visitTypeTest(JCInstanceOf that) {
        ...
        if (pureCopy || rac) {
            ...
            // FIXME - ESC - not yet implemented as an assertion
        }
    }
    ...
    public void visitIndexed(JCArrayAccess that) {
        ...
        if (javaChecks) {
            ...
            if (translatingJML) {
                ...
                addAssert(that.pos(),Label.UNDEFINED_NULL,nonnull);
            } else {
                addAssert(that.pos(),Label.POSSIBLY_NULL,nonnull);
            }
        }
    ...
    public void visitSelect(JCFieldAccess that) {
        ...
        if (pureCopy) {
            ...
            if (!that.sym.isStatic()) {
                ...
                if (javaChecks) addAssert(nonnull.pos(),
    ...
    public void visitIdent(JCIdent that) {
        ...
        // Check if the id is a model field, and if so, map it to a call of
        ...
        if (rac && attr.isModel(that.sym) && that.sym instanceof VarSymbol) {
            Name mmName = names.fromString(Strings.modelFieldMethodPrefix + that.name.toString());
        ...
        if (actual != null) {
            ...
            result = eresult = actual == that ? actual : convertExpr(actual);
            ...
            JCIdent id = treeutils.makeIdent(that.pos, currentThisSym);
            result = eresult = id;
...
//        addStat(M.at(that.pos()).ForeachLoop(v, e, b));
    ...
    public void visitJmlMethodDecl(JmlMethodDecl that) {
        ...
        //if (attr.isModel(that.sym)) translatingJML = true;
        try {
        ...
        try {
          switch (that.token) {
            case ASSERT:
                ...
                result = addAssert(that.pos(),Label.EXPLICIT_ASSERT,e,null,null,that.optionalExpression);
                break;
            case ASSUME:
                addStat(comment(that));
    ...
    public void visitJmlVariableDecl(JmlVariableDecl that) {
        ...
        if (JmlAttr.instance(context).isGhost(that.mods)) {
            try {
                ...
                // If the following condition is true, then the declaration is
                // a class declaration, not within a method
                if (currentStatements == null) { pushBlock(); inClassDecl = true; }
