...
public class JmlAssertionAdder extends JmlTreeScanner {
    ...
    protected JCBlock convertBody() {
        ...
        if (isConstructor) {
            ...
            newstats.addAll(decl.body.stats);
            ...
            return M.at(decl.body.pos()).Block(decl.body.flags,newstats.toList());
    ...
    protected JCExpression methodCallUtilsExpression(DiagnosticPosition pos, String methodName, JCExpression ... translatedArgs) {
        ...
        JCMethodInvocation c = M.at(pos.getPreferredPosition()).Apply(null,m,args);
...
//        d.sym.flags_field |= Flags.FINAL;
    ...
    protected void addPrePostConditions(JCMethodDecl decl, 
        ...
        java.util.List<ClassSymbol> parents = Utils.instance(context).parents((ClassSymbol)decl.sym.owner);
        
        currentStatements = preStats;
        if (!isConstructor) {

            for (ClassSymbol csym: parents) { // FIXME - check visibility
        ...
        for (MethodSymbol msym: Utils.instance(context).parents(decl.sym)) {
        ...
        if (pureCopy) {
            ...
            try {
                if (tag == JCTree.AND) {
                    ...
                    rhs = scanExpr(rhs); // condition is used within scanExpr so this statement must follow the previous one
...
//  }
  ...
  public void translateTypeOf(JCMethodInvocation tree) {
      ...
      switch (tag) {
          case TypeTags.ARRAY:
          case TypeTags.CLASS:
              eresult = methodCallgetClass(scanExpr(arg));
              break;
  ...
  public void translateElemtype(JCMethodInvocation tree) {
      ...
      Symbol ms = e.sym;
      JCFieldAccess m = M.Select(scanExpr(arg),n);
      m.sym = ms;
      m.type = m.sym.type;
