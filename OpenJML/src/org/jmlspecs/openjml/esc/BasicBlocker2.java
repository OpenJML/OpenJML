...
//        context.put(key, this);
    ...
    public class VarMap {
        ...
        /** Returns the name for a class symbol as stored in this map; returns null
    ...
    protected void shouldNotBeCalled(JCTree that) {
        Log.instance(context).error("esc.internal.error","Did not expect to be calling a " + that.getClass() + " within BasicBlocker2");
    ...
    protected void startBlock(@NonNull BasicBlock b) {
        ...
        if (nm.endsWith(FINALLY)) {
            ...
            // exit the finally block and go to whatever enclosing finally blocks
            // there are.
            BasicBlock bb = finallyStack.remove(0);
            if (bb != b) {
                log.warning("esc.internal.error","Mismatched finally blocks: " + b.id.name + " " + bb.id.name);
            }
...
//        }
        ...
        // They should be in partial order
...
//            JCVariableDecl decl = makeVariableDecl(name,syms.intType,treeutils.makeIntLiteral(0,pos),pos);
    ...
    public void visitSwitch(JCSwitch that) { 
        ...
        try {
            ...
            Name switchName = names.fromString("_switchExpression_" + pos);
    ...
    public void visitTry(JCTry that) {
        ...
        if (finallyStat != null) finallyBlock.statements.add(finallyStat); // it gets the (unprocessed) statements of the finally statement
    ...
    public void visitReturn(JCReturn that) {
        if (finallyStack.isEmpty()) {
            // This is the case only because JmlAssertionAdder wraps everything in a try-finally statement
            Log.instance(context).warning("esc.internal.error","finally stack is unexpectedly empty");
        } else {
        ...
        if (that.token == JmlToken.BSOLD || that.token == JmlToken.BSPRE) {
            ...
            try {
                if (that.args.size() == 1) {
                    ...
                    if (currentMap == null) {
                        // FIXME Internal error - no map for the label
                    }
                    that.args.get(0).accept(this);
...
//        }
    ...
    public void visitJmlStatementExpr(JmlStatementExpr that) { 
        if (that.token == JmlToken.COMMENT) {
            ...
            log.error("esc.internal.error","Unknown token in BasicBlocker2.visitJmlStatementExpr: " + that.token.internedName());
        }
    }
    ...
    //     JCIdent nodes within their scopes using the correct signle-assignment names
