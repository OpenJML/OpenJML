...
import org.jmlspecs.openjml.jmldoc.*;
...
public class ClassWriterJml extends ClassWriterImpl {
    ...
    public void writeClassSpecs() {
        ...
        if (classDoc instanceof ClassDocImpl) {
            ...
            if (hsp || ss.length()!=0){
                printSpecHeader("JML Class Specifications");
                if (ss.length()!=0) {
                    strong("Annotations:");
                    print(ss);
                    br();
    ...
    public void printInheritedSpecs(Context context, ClassSymbol csym) {
        ...
        if (hasSpecsToPrint(tspecs)){
            strong("Class Specifications inherited from " + csym + ": ");
            printSpecs(tspecs);
        }
    }
    ...
    public void printHtmlJmlHeader() {
