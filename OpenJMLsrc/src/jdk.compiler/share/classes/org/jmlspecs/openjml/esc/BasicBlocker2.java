...
public class BasicBlocker2 extends BasicBlockerParent<BasicProgram.BasicBlock,BasicProgram>  {
    ...
    protected void addAssert(Label label, JCExpression trExpr, int declpos, List<JCStatement> statements, int usepos, JavaFileObject source, JCTree statement) {
        JmlTree.JmlStatementExpr st = factory.at(statement.pos()).JmlStatementExpr(assertID, assertClause,label,trExpr);
...
//    
    ...
    public static class Array2Type extends Type {
        ...
        public Array2Type(Type i, Type c) { super(null,null); indexType = i; componentType = c; }
        @Override
...
//        log.noticeWriter.println("MAP FOR BLOCK " + block.id + JmlTree.eol + newMap.toString());
    ...
    public void visitApply(JCMethodInvocation that) { 
        ...
        if (that.meth instanceof JCFieldAccess) {
            ...
            utils.warning("esc.not.implemented","BasicBlocker2.visitApply for " + that);
            msym = null;
            obj = null;
            result = trueLiteral;
            return;
...
//    }
    ...
    public void visitJmlStatementExpr(JmlStatementExpr that) { 
        if (that.clauseType == commentClause) {
            ...
            JmlStatementExpr st = M.at(that.pos()).JmlStatementExpr(that.clauseType.keyword(),that.clauseType,that.label,ex);
            st.id = that.id;
            ...
            if (stat instanceof JmlStatementExpr) {
                ...
                if (label == Label.METHOD_ASSUME) {
                    ...
                    if (mm == msym) {
                        ...
                        JmlStatementExpr st = M.at(Position.NOPOS).JmlStatementExpr(assumeID, assumeClause,Label.DSA,ex);
                        currentBlock.statements.add(st);
                        return ;
                    }
...
//        }
    ...
    @Override public void visitTypeTest(JCInstanceOf that) { 
        that.expr = convertExpr(that.expr);
        scan(that.pattern); // FIXME - if the type tree is rewritten, we are not capturing the result
        result = that; 
    }
