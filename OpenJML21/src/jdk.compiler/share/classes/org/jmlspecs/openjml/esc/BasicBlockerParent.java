...
abstract public class BasicBlockerParent<T extends BlockParent<T>, P extends BasicProgramParent<T>> extends JmlTreeScanner {
    ...
    protected void processBlock(/*@non_null*/ T block) {
        ...
        if (!program.blocks.contains(block)) {
            ...
            utils.warning("jml.internal","Basic block " + block.id + " is being re-processed");
        }
    }
    ...
    protected void setCurrentBlock(T b) {
        if (currentBlock != null) {
        	utils.warning("jml.internal.notsobad","Starting block " + b.id + " when " + currentBlock.id + " is not completed"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
        }
        currentBlock = b;
...
//        processBlock(afterIf);
    ...
    public void visitBreak(JCBreak that) { 
        if (that.label != null) {
            ...
            if (breakBlock == null) {
                utils.error(that.pos(),"jml.internal","No target found for break label " + that.label);
            } else {
                replaceFollows(currentBlock,breakBlock);
            }
        } else if (breakStack.isEmpty()) {
        	utils.error(that.pos(),"jml.internal","Empty break stack");
            ...
            if (tree instanceof JCSwitch) {
                ...
                if (b == null) {
                	utils.error(that.pos(),"jml.internal","Corresponding break target is not found"); //$NON-NLS-1$ //$NON-NLS-2$
                } else {
        ...
        if (!remainingStatements.isEmpty()) {
        	utils.warning(remainingStatements.get(0),"jml.internal.notsobad","Statements after a break statement are unreachable and are ignored"); //$NON-NLS-1$ //$NON-NLS-2$
        }
    }
    ...
    public void visitContinue(JCContinue that) {
        ...
        if (loopStack.isEmpty()) {
        	utils.error(that,"jml.internal","Empty loop stack for continue statement"); //$NON-NLS-1$ //$NON-NLS-2$
            ...
            if (b == null) utils.error(that,"jml.internal","No continue block found to match this continue statement");  //$NON-NLS-1$//$NON-NLS-2$
            ...
            String blockName = blockNamePrefix(t.pos,LOOPCONTINUE);
            T b = blockLookup.get(blockName);
            if (b == null) utils.error(that,"jml.internal","No continue block found to match this continue statement, with label ", that.label); //$NON-NLS-1$ //$NON-NLS-2$
            else replaceFollows(currentBlock,b);
        }
        if (!remainingStatements.isEmpty()) {
        	utils.warning(remainingStatements.get(0),"jml.internal.notsobad","Statements after a continue statement are unreachable and are ignored"); //$NON-NLS-1$ //$NON-NLS-2$
        }
    }
    ...
    public void visitReturn(JCReturn that) {
        if (!remainingStatements.isEmpty()) {
            ...
            if (!remainingStatements.isEmpty()) {
                ...
                if (continuation == Continuation.CONTINUE) utils.warning(remainingStatements.get(0).pos,
        ...
        if (finallyStack.isEmpty()) {
        	...
        	utils.warning(that,"esc.internal.error","finally stack is unexpectedly empty");  //$NON-NLS-1$//$NON-NLS-2$
        } else {
    ...
    public void visitThrow(JCThrow that) { 
        ...
        if (finallyStack.isEmpty()) {
            ...
            // JmlAssertionAdder wraps everything in a try-finally statement
            // TODO - generalize this
        	utils.warning(that,"esc.internal.error","finally stack is unexpectedly empty");
        } else {
