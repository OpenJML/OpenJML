package org.jmlspecs.openjmltest.testsuites;

import org.jmlspecs.openjmltest.TCBase;
import org.junit.*;

@org.junit.FixMethodOrder(org.junit.runners.MethodSorters.NAME_ASCENDING)
public class flow extends TCBase {

    /** Forward reference from invariant is OK */
    @Test
    public void testForwardReference() {
        helpTCText(null, "public class TEST { \n//@ invariant b;\n  boolean b;}");
    }

    /** Forward reference in model class */
    @Test
    public void testForwardReference2() {
        addMockFile("$A/A.jml","public class A { }\n//@ model class B { int b = c; int c = 0; }\n\n");
        helpTCText("A.java","public class A { }"
        ,"/$A/A.jml:2: error: illegal forward reference",29
        );
    }

    /** Flow checks for model methods*/
    @Test
    public void testModelMethod() {
        addMockFile("$A/A.jml","public class A { \n//@ model int m() {} \n}");
        helpTCText("A.java","public class A { int mm() {} }"
                ,"/A.java:1: error: missing return statement",28
                ,"/$A/A.jml:2: error: missing return statement",20
        );
    }

    /** Check on name of file  - not particularly a flow check */
    @Test
    public void testFileName() {
        helpTCText("A.java","public class B { }"
        ,"/A.java:1: error: class B is public, should be declared in a file named B.java",8
        );
    }

    /** Check on name of file */
    @Test
    public void testFileName3() {
        helpTCText("A.java","public class A { } //@ model public class B {}"
        ,"/A.java:1: error: class B is public, should be declared in a file named B.java",37
        );
    }

    /** Check on name of file  - not particularly a flow check */
    @Test
    public void testFileNameModel() {
        helpTCText("A.java","/*@ model public class B { } */"
        ,"/A.java:1: error: class B is public, should be declared in a file named B.java",18
        );
    }

    /** Flow checks for ghost fields */
    @Test
    public void testGhostForwardReference() {
        addMockFile("$A/A.jml","public class A { \n//@ ghost int iiii = jjjj; ghost int jjjj; \n}");
        helpTCText("A.java","public class A { int ii = jj; int jj;}"
                ,"/A.java:1: error: illegal forward reference",27
                ,"/$A/A.jml:2: error: illegal forward reference",22
        );
    }
    
    /** Flow checks for quantified expression */
    @Test
    public void testQuantifiedFlow() {
        helpTCText("A.java","public class A { \n"
                +" public static void m() {\n"
                +"  //@ ghost \\bigint n = (\\num_of int i; 0<i && i<5; n>i);\n" // ERROR - can't use n in the initializer
                +"}}"
                ,"/A.java:3: error: variable n might not have been initialized", 53
                );
                
    }

    /** Flow checks for a non-executable quantified expression */
    @Test
    public void testQuantifiedNonExFlow() {
        helpTCText("A.java","public class A { \n"
                +" public static void m() {\n"
                +"  //@ ghost \\bigint n = (\\num_of int i; ; n>i);\n" // ERROR - can't use n in the initializer
                +"}}"
                ,"/A.java:3: error: variable n might not have been initialized", 43
                );
                
    }
}
