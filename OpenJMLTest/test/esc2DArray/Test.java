public class Test {
    ...
    //@ requires m >= 1 && n>= 0;  // FIXME - fails to prove if m is allowed to be 0
    ...
    public static int[][] m(int m, int n) {
        ...
        //@ assert a != null;
        //@ assert a.length == m;
        //@ assume m > 0 ==> a[0] != null;  // FIXME - proof fails if the following are not assumed
        //@ assume m > 0 ==> a[0].length == n;
        //@ assume \forall int i; 0 <= i < m; a[i] != null && a[i].length == n;
        //@ assume \forall int e; 0<=e<m; \forall int k; 0 <= k < m; (e != k ==> a[e] != a[k]);
