...
public /*@ nullable_by_default @*/ class BinaryTree {
    ...
    /*@ normal_behaviour
      ...
      @   ensures \result == checkNulls(n, depth, depth+1);
    ...
    /*@ ensures \fresh(\result);
      ...
      @ ensures checkNulls(n, \result.min, \result.max);
      @ pure
        non_null */
    ...
    /*@
      requires min <= max;
      ensures \result == (n == null ? min <= 0 <= max : (checkNulls(n.left, min-1, max-1) & checkNulls(n.right, min-1, max-1)));
      spec_pure
      model public boolean checkNulls(Node n, int min, int max);
     */
