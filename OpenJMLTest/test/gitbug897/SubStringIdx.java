...
public class SubStringIdx {
    ...
    public static int substringIdx(String pat, String text) {
        ...
        //@ maintaining 0 <= i <= text.length() - pat.length() + 1;
        //CANNOT PROVE@ maintaining \forall int k; 0 <= k < i; (\exists int l; 0 <= l < pat.length(); text.charAt(k+l) != pat.charAt(l));
        //@ loop_writes i;
    ...
    public void m(String pat, String text) {
        //@ ghost boolean b1 = pat != null && text != null && \exists int i; 0 <= i <= text.length() - pat.length(); (\forall int j; 0 <= j < pat.length(); text.charAt(i+j) == pat.charAt(j));
        //@ ghost boolean b2 = pat != null && text != null && !\exists int i; 0 <= i <= text.length() - pat.length(); (\forall int j; 0 <= j < pat.length(); text.charAt(i+j) == pat.charAt(j));
        //@ assert !(b1 && b2);
    }

    //@  requires pat != null && text != null;
    //@  old boolean b = \exists int i; 0 <= i <= text.length() - pat.length(); (\forall int j; 0 <= j < pat.length(); text.charAt(i+j) == pat.charAt(j));
    //@  {|
    //@     requires b;
    //@  also
    //@     requires !b;
