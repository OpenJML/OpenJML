 ...
 * via a properties file listed in {@link OpenJMLSettings#toolOptions}
 * (as {@code ["--properties", "<path>"]}).
 *
 * <p>Each test writes a small {@code .properties} file using
 * {@code org.openjml.option.<flag>=<value>} entries, sets it as
 * {@code toolOptions = ["--properties", path]}, and then verifies
...
public class PropertiesFileOptionsTest extends LspTestBase {
     ...
     * clean way to verify the properties file is applied: the same source
     * produces different ESC results depending on whether the key is active.
     *
     * <p>Source layout:
     * <pre>
     ...
     *
     * With {@code org.openjml.option.keys=MYKEY} in the properties file
     * (passed via {@code toolOptions}): the annotation IS active,
     * {@code ensures false} is imposed on a normally-terminating method,
     * and ESC reports a postcondition violation.
     */
    @Test
    public void testOptionalKeyViaPropertiesFile() throws Exception {
        ...
        OpenJMLSettings settings = settingsWithPropertiesFile(propsPath);
     ...
     * the properties file, OpenJML should report the infeasibility.
     */
    @Test
    ...
    /**
     * Verifies that boolean options are correctly applied via a properties file
     * in {@code toolOptions}.  {@code --require-white-space} is a boolean flag:
     * when set, JML comments of the form {@code //@keyword} (no space after
     * {@code @}) are treated as ordinary Java comments and ignored.
     *
     * <p>Source layout:
     * <pre>
     ...
     * With {@code org.openjml.option.require-white-space=true} in the properties
     * file the comment is silently ignored and ESC finds nothing.
     */
    @Test
    ...
    // -----------------------------------------------------------------------
    // Test 4: second --properties in toolOptions overrides the first
    // -----------------------------------------------------------------------

    /**
     * When {@code toolOptions} contains two {@code --properties} entries the
     * second file overrides values set by the first, because OpenJML processes
     * options in order (last value wins for repeated keys).
     *
     * <p>This test verifies the ordering with {@code --check-feasibility}:
     * <ul>
     *   <li>The first file enables feasibility checking
     *       ({@code check-feasibility=basic}), which would produce a diagnostic
     *       for {@code //@ requires false;}.</li>
     *   <li>The second file resets it to {@code none} (the default),
     *       so no feasibility diagnostic should appear.</li>
     * </ul>
     * The expected result is zero diagnostics, confirming the second file won.
     */
    @Test
    public void testSecondPropertiesFileOverridesFirst() throws Exception {
        String source =
        ...
        String firstPath = writeTempProps("first.properties",
                "org.openjml.option.check-feasibility=basic\n");
        String secondPath = writeTempProps("second.properties",
        ...
        assertTrue("Expected no ESC diagnostics — second properties file (check-feasibility=none) "
                + "should override the first file's check-feasibility=basic",
                diags.isEmpty());
    }
