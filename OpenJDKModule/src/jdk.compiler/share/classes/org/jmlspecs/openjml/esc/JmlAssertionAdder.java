...
//                notImplemented(receiver, "receiver of class " + (receiver == null ? "-null-" : receiver.getClass().getName()));
    ...
    protected void assertInvariants(JCExpression expr, JCExpression currentThis) {
        ...
        try {
            for (JmlTypeClause t: specs.initializeAndGetSpecs((ClassSymbol)expr.type.tsym).clauses) {
    ...
    protected void addInvariants(boolean assume, DiagnosticPosition d, boolean staticOnly, boolean fieldInvariants, boolean helper, ClassSymbol csym, JCExpression currentThis) {
        ...
        ListBuffer<JCStatement> check = pushBlock();
        try {
            for (JmlTypeClause t: specs.initializeAndGetSpecs(csym).clauses) {
                if (t.clauseType != invariantClause) continue;
        ...
        try {
            if (rac) {
                ...
                while (iter.hasPrevious()) {
                    JmlSpecs.TypeSpecs tyspecs = specs.initializeAndGetSpecs((ClassSymbol)iter.previous().tsym);
...
//                }
            ...
            if (esc) {
                ...
                while (iter.hasPrevious()) {
                    ...
                    TypeSpecs tspecs = specs.initializeAndGetSpecs((ClassSymbol)ty);
...
//                                        addAssume(that,Label.IMPLICIT_ASSUME,e);
    ...
    protected void addFinalStaticField(JCFieldAccess convertedfa) {
        ...
        if (value != null) {
            ...
            if (JmlOption.isOption(context,JmlOption.STATIC_INIT_WARNING)) {
                if (!hasStaticInitializer((ClassSymbol)convertedfa.sym.owner) && !convertedfa.sym.isEnum()
                    ...
                    utils.warning(convertedfa, "jml.message", "Use a static_initializer clause to specify the values of static final fields: " + utils.qualifiedName(convertedfa.sym));
                    if (utils.jmlverbose >= Utils.JMLVERBOSE) utils.note(convertedfa, "jml.message", "Warned about a non-constant initialized static final field: " + st.toString());
                }
            }
        ...
        if (st != null) {
            ...
            discoveredFields.stats = discoveredFields.stats.append(st);
            if (utils.jmlverbose >= Utils.JMLVERBOSE) utils.note(convertedfa, "jml.message", "Added an initalized static final field: " + st.toString());
        }
    ...
    public boolean hasStaticInitializer(JmlClassDecl decl) {
        JmlSpecs.TypeSpecs ts = specs.initializeAndGetSpecs(decl.sym);
        return ts.staticInitializerSpec != null;
    }

    public boolean hasStaticInitializer(ClassSymbol sym) {
        JmlSpecs.TypeSpecs ts = specs.initializeAndGetSpecs(sym);
        return ts.staticInitializerSpec != null;
    }
...
//                                    JCBlock elsebl = popBlock(ex);
    ...
    protected void assumeFieldInvariants(ClassSymbol classSym, boolean staticOnly, boolean noFinal) {
        ...
        JCExpression savedThis = currentThisExpr;
        JmlSpecs.TypeSpecs cspecs = specs.initializeAndGetSpecs(classSym); 
...
//                    JCExpression cond;
//                    if (cs.pats != null) cond = treeutils.makeEquality(cs.pos,selector,convertExpr(cs.pat));
//                    else {
//                        // default
//                        cond = null;
//                        for (JCCase c: that.cases) {
...
//            result = addStat(M.at(that).Throw(expr).setType(that.type));
    ...
    java.util.List<JCFieldAccess> datagroupContents(JCFieldAccess fa, ClassSymbol csym) {
        ...
        for (Type parent:  parents(csym.type,false)) {
            ...
            TypeSpecs tspecs = JmlSpecs.instance(context).initializeAndGetSpecs((ClassSymbol)parent.tsym);
    ...
    JCExpression accessAllowed(JmlStoreRefKeyword storeref, JCExpression pstoreref) {
        ...
        utils.error(pos,"esc.not.implemented", "Assignability comparison: " + storeref + " vs. " + pstoreref);
...
//            }
        ...
        utils.error(pos,"esc.not.implemented", "Assignability comparison: " + fa + " vs. " + pfac);
        return treeutils.falseLit;
    }
...
//                                            }
                        ...
                        if (useDefault && !translatingJML && cs.token != modelprogramClause && cs.block == null && !inliningCall) {
                            utils.warning(cs,"jml.internal","Unexpected absence of an assignable clause after desugaring: " + mpsym.owner + " " + mpsym.toString());
                        }
                      } finally {
                          JCBlock bl = popBlock(cs);
            ...
            for (Pair<MethodSymbol,Type> pair: overridden) {
                ...
                for (JmlSpecificationCase cs : calleeSpecs.cases) {
                    ...
                    if (cs.block != null && !localVariables.isEmpty()) {
                    	utils.warning(cs.block.pos, "jml.message", "Cannot use functions with model methods within quantified expressions");
                        break;
                    }
                    if (cs.block != null)  { // Note: inlining for RAC, also -- FIXME - need to check this
                        JCExpression pre = preExpressions.get(cs);
...
//                return expr; // - need to do casts explicitly at least for op= operations
        ...
        if (types.isSameType(newtype,expr.type.stripMetadata())) return expr;
...
//            }
    ...
    protected JCExpression createUnboxingExpr(JCExpression expr) {
        ...
        if (rac || useMethod) {
            ...
            if (fsym == null) {
            	utils.error(expr, "jml.message", "Could not find model field " + fieldName);
                // ERROR - throw something
            }
...
//            if (!translatingJML) addBinaryChecks(that,optag,lhs,rhs,null);
    ...
    public JCExpression makeBin(JCTree that, JCTree.Tag tag, OperatorSymbol opSym, JCExpression lhs, JCExpression rhs, Type maxJmlType) {
        ...
        if (javaChecks && splitExpressions && !pureCopy && localVariables.isEmpty()) {
            ...
            if (translatingJML) {
                ...
                if (allow && forbid || allow && ignore || forbid && ignore) {
                    utils.warning(p, "jml.message", "It is not permitted to have more than one of allow, forbid, ignore for one line");
                }
                if (allow) {
                    useException = true;
                } else if (forbid) {
...
//                    }
    ...
    public void visitIdent(JCIdent that) {
        ...
        if (!rac && sym != null && alreadyDiscoveredFields.add(sym)) { // true if s was NOT in the set already
            ...
            } else if (sym.kind == Kinds.Kind.VAR && sym.owner != null && sym.owner.kind == Kinds.Kind.MTH && splitExpressions && !localVariables.containsKey(sym) && !isFormal(sym,(MethodSymbol)sym.owner)) {
                addLocalVar(that);
            } else {
                utils.note(true, "No invariants created for " + that);
            }
...
//        }
    ...
    /*@nullable*/ JCExpression getInvariant(/*@non_null*/ DiagnosticPosition pos, /*@non_null*/ Type base, /*@non_null*/ Type t, /*@nullable*/ JCExpression obj) {
        ...
        TypeSpecs tspecs = specs.initializeAndGetSpecs((ClassSymbol)t.tsym);
        JCExpression saved = currentThisExpr;
...
//        if (!rac) classDefs.add(cl);// FIXME - should we have this at all?
    ...
    public void visitJmlTypeClauseRepresents(JmlTypeClauseRepresents that) {
        ...
        JmlSpecs.TypeSpecs typeSpecs = specs.initializeAndGetSpecs(classDecl.sym);
        if (sym != null) {
...
//                }
    ...
    public static <F,S> Pair<F,S> pair(F f, S s) { return new Pair<F,S>(f,s); }
...
//                }
                ...
                for (JCStatement st: that.defs) {
                    if (st instanceof JCVariableDecl) replacements.remove(((JCVariableDecl)st).sym);
                }
            }
        }
