...
// Command Line Options: /nologo -nologo /nologo /prover:blank /nologo /print:boogie_tmp.@TIME@.bpl /nologo /proverLog:boogie_tmp.@TIME@.simplify /nologo /nologo /nologo /nologo /nologo Bug163
...
{
  var this: struct where true, return.value: struct where true, SS$Display.Return.Local: struct where true, stack0s: struct;
  ...
  block2431:
    ...
    // ----- copy  ----- Bug163.ssc(6,4)
    stack0s := return.value;
    // ----- return  ----- Bug163.ssc(6,4)
    this$out := this;
    $result := stack0s;
    return;
...
{
  var this: struct where true, s: struct where true, stack0o: ref, return.value: int where InRange(return.value, System.Int32), SS$Display.Return.Local: int where InRange(SS$Display.Return.Local, System.Int32), stack0i: int;
  ...
  block2686:
    // ----- load address  ----- Bug163.ssc(15,16)
    havoc stack0o;
    // ----- load field  ----- Bug163.ssc(15,9)
    return.value := cast($StructGet(s, S2.x),int);
    // ----- branch
    goto block2703;
  ...
  block2720:
    // ----- copy
    SS$Display.Return.Local := return.value;
    // ----- copy  ----- Bug163.ssc(16,5)
    stack0i := return.value;
    // ----- return  ----- Bug163.ssc(16,5)
    this$out := this;
    $result := stack0i;
    return;
