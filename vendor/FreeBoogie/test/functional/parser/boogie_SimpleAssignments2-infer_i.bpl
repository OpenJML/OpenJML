...
// Command Line Options: -nologo /prover:blank /print:boogie_tmp.@TIME@.bpl /proverLog:boogie_tmp.@TIME@.simplify SimpleAssignments2.dll /infer:i
...
{
  var zero: int where InRange(zero, System.Int32), stack0i: int, x: int where InRange(x, System.Int32), return.value: int where InRange(return.value, System.Int32), SS$Display.Return.Local: int where InRange(SS$Display.Return.Local, System.Int32);

  entry:
    assume $IsNotNull(this, SimpleAssignments2);
  ...
  block1173:
    // ----- load constant 0  ----- SimpleAssignments2.ssc(7,3)
    zero := 0;
    // ----- load constant 1  ----- SimpleAssignments2.ssc(8,3)
    stack0i := 1;
    // ----- binary operator  ----- SimpleAssignments2.ssc(8,3)
    assert zero != 0;
    stack0i := stack0i / zero;
    // ----- copy  ----- SimpleAssignments2.ssc(8,3)
    x := stack0i;
    // ----- copy  ----- SimpleAssignments2.ssc(10,3)
    return.value := x;
    // ----- branch
    goto block1190;
  ...
  block1190:
    ...
    // ----- copy  ----- SimpleAssignments2.ssc(11,2)
    stack0i := return.value;
    // ----- return  ----- SimpleAssignments2.ssc(11,2)
    $result := stack0i;
    return;
...
implementation SimpleAssignments2..ctor(this: ref)
{
  ...
  block1496:
    // ----- call  ----- SimpleAssignments2.ssc(3,7)
    assert this != null;
    call System.Object..ctor(this);
    // ----- return
    assert this != null;
    assert $Heap[this, $inv] == System.Object && $Heap[this, $localinv] == $typeof(this);
    assert (forall $p: ref :: $p != null && $Heap[$p, $allocated] == true && $Heap[$p, $ownerRef] == this && $Heap[$p, $ownerFrame] == SimpleAssignments2 ==> $Heap[$p, $inv] == $typeof($p) && $Heap[$p, $localinv] == $typeof($p));
    $Heap[this, $inv] := SimpleAssignments2;
    assume IsHeap($Heap);
    return;
