...
// Command Line Options: /nologo -nologo /nologo /prover:blank /nologo /print:boogie_tmp.@TIME@.bpl /nologo /proverLog:boogie_tmp.@TIME@.simplify /nologo /nologo /nologo /nologo /nologo Literals
...
function #Test.Foo$System.String$notnull([ref,<x>name]x, ref) returns (bool);

function #Test.Quux$System.Int64([ref,<x>name]x, int) returns (bool);
...
axiom (forall $Heap: [ref,<x>name]x, s$in: ref :: { #Test.Foo$System.String$notnull($Heap, s$in) } IsHeap($Heap) && $IsNotNull(s$in, System.String) && (forall $pc: ref :: $pc != null && $Heap[$pc, $allocated] == true && $Heap[$pc, $ownerRef] == $Heap[s$in, $ownerRef] && $Heap[$pc, $ownerFrame] == $Heap[s$in, $ownerFrame] ==> $Heap[$pc, $inv] == $typeof($pc) && $Heap[$pc, $localinv] == $typeof($pc)) ==> true && #Test.Foo$System.String$notnull($Heap, s$in) == true);
...
{
  var s: ref where $IsNotNull(s, System.String), return.value: bool where true, SS$Display.Return.Local: bool where true, stack0b: bool;
  ...
  block2516:
    // ----- load constant True  ----- Literals.ssc(3,60)
    return.value := true;
    // ----- branch
    goto block2533;
  ...
  block2550:
    ...
    // ----- copy  ----- Literals.ssc(3,73)
    stack0b := return.value;
    // ----- return  ----- Literals.ssc(3,73)
    $result := stack0b;
    return;
...
axiom (forall $Heap: [ref,<x>name]x, l$in: int :: { #Test.Quux$System.Int64($Heap, l$in) } IsHeap($Heap) && InRange(l$in, System.Int64) ==> true && #Test.Quux$System.Int64($Heap, l$in) == true);
...
{
  var l: int where InRange(l, System.Int64), return.value: bool where true, SS$Display.Return.Local: bool where true, stack0b: bool;
  ...
  block2805:
    // ----- load constant True  ----- Literals.ssc(5,58)
    return.value := true;
    // ----- branch
    goto block2822;
  ...
  block2839:
    // ----- copy
    SS$Display.Return.Local := return.value;
    // ----- copy  ----- Literals.ssc(5,71)
    stack0b := return.value;
    // ----- return  ----- Literals.ssc(5,71)
    $result := stack0b;
    return;
...
{
  var x: real where true, y: real where true, stack0r: real, return.value: real where true, SS$Display.Return.Local: real where true;
  ...
  block4097:
    // ----- binary operator  ----- Literals.ssc(16,14)
    stack0r := #rdiv(x, y);
    // ----- copy  ----- Literals.ssc(16,7)
    return.value := stack0r;
    // ----- branch
    goto block4114;
  ...
  block4131:
    // ----- copy
    SS$Display.Return.Local := return.value;
    // ----- copy  ----- Literals.ssc(17,4)
    stack0r := return.value;
    // ----- return  ----- Literals.ssc(17,4)
    $result := stack0r;
    return;
...
axiom (forall heap: [ref,<x>name]x :: { heap[$stringLiteral0, $allocated] } IsHeap(heap) ==> heap[$stringLiteral0, $allocated]) && $IsNotNull($stringLiteral0, System.String) && $StringLength($stringLiteral0) == 3;
